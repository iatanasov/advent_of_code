...
pub fn part1(content: String) -> Result<(), Report> {
    ...
    for (_, [seed]) in  seeds_regex().captures_iter(lines.next().unwrap()).map(|c| c.extract()) {
...
fn exclusive_found_in_range(sorted_ranges: &Vec<(usize, usize,usize)>, val: usize) -> (usize, usize) {
